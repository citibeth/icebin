...
Hntr::Hntr(HntrGrid const &_A, HntrGrid const &_B, double _DATMIS)
    : A(_A), B(_B),
      SINA(Range(0 Agrid.size()-1)),
      SINB(Range(0, Bgrid.size()-1)),
...
void Hntr::parition_east_west()
{
    ...
    for (int IBp1=1; IBP1 <= Bgrid.im; ++IBp1) {
        RIB = (IBp1-1+Bgrid.offi)*Agrid.im;    //  scaled longitude of eastern edge
        while (RIA < RIB) {
            IA  += 1;
            RIA += DIA;
        }
...
void Hntr::partition_north_south()
{
    ...
    for (int JA=1; JA <= Agrid.jm-1; ++JA) {
        RJA = (JA + .5-FJEQA) * Agrid.dlat;  //  latitude in minutes of northern edge
    ...
    // -----------
    FJEQB = .5*(1+Bgrid.jm);
    for (JB=1; JB <= Bgrid.jm-1; ++JB) {
        RJB = (JB+.5-FJEQB)*Bgrid.dlat;  //  latitude in minutes of northern edge
    ...
    blitz::Array<double,1> &B)
{
    ...
    if ((WTA_c.extent(0) != A.size()) ||
        (A_c.extent(0) != A.size()) ||
        (B_c.extent(0) != B.size()))
    {
        (*icebin_error)(-1, "Error in dimensions: (%d, %d, %d) vs. (%d, %d)\n",
            WTA_c.extent(0), A_c.extent(0), B_c.extent(0),
            A.size(), B.size());
    }

#if 0
    // Convert to Fortran-style (1-base) arrays, which
    // this code is set up to use.
    blitz::Array<double,1> const WTA(c_to_f(WTA_c));
    blitz::Array<double,1> const A(c_to_f(A_c));
    blitz::Array<double,1> const B(c_to_f(B_c));
#endif
    ...
    blitz::Array<double,1> &B)
{
    ...
    for (int JB=1; JB <= Bgrid.jm; JM += Bgrid.jm-1) {
        double BMEAN  = DATMIS;
        double WEIGHT = 0;
        double VALUE  = 0;
        for (int IB=1; ; ++IB) {
